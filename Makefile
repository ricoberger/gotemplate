...
PWD         ?= $(shell pwd)
REPO        ?= github.com/ricoberger/gotemplate
...
docker-build:
	for target in $(WHAT); do \
		docker build -f cmd/$$target/Dockerfile -t "${DOCKER_REPO}/$$target:${VERSION}" --build-arg REVISION=${REVISION} --build-arg VERSION=${VERSION} .; \
	done

docker-publish:
	for target in $(WHAT); do \
		docker push "${DOCKER_REPO}/$$target:${VERSION}"; \
	done

release: clean build-darwin-amd64 build-linux-amd64 build-windows-amd64 docker-build docker-publish
